...
public record SubjectView(Long id, String name, String description, int orderNo,
    ...
    public record SessionView(Long id, int seq, String name, LocalDate lessonDate, Integer learningMinutes) {
        public static SessionView of(Session s) {
            return new SessionView(s.getId(), s.getSeq(), s.getName(), s.getLessonDate(), s.getLearningMinutes());
        }
    }
}
