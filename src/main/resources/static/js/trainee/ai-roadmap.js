/**
 * ════════════════════════════════════════════════════════════════════════
 *  [기능 1] AI 기반 직무 로드맵                        화면: /trainee/ai/roadmap
 * ════════════════════════════════════════════════════════════════════════
 *
 * ■ 무엇을 하는가
 *   채용공고를 주 1회 수집해 직무별 요구 역량을 뽑고, 훈련생이 이미 학습한 것과
 *   비교해 "다음에 뭘 채워야 하는지"를 단계로 보여준다.
 *
 * ■ 어떻게 구현했는가
 *   1) 직무 탭 — 수집된 공고를 직무로 묶는다. 탭을 바꾸면 아래 전체가 다시 그려진다.
 *   2) 지도(ai-roadmap-map.js) — 단계를 길 위의 지점으로 그린다. 목록으로 세우면
 *      "어디까지 왔는지"가 안 보인다. done/current/locked 세 상태만 쓴다.
 *   3) 역량 미터 — 공고 요구 항목 대비 내 학습 이력의 충족도.
 *   4) 원문 공고 목록 — 판단 근거를 감추지 않는다. 수집일을 항상 같이 보여준다.
 *
 * ■ 데이터
 *   ⚠ 전부 시연용(아래 DUMMY). 공고 수집기가 아직 없다.
 *     외부 회사명·공고는 실제 수집 결과가 들어올 자리라 가상 이름을 쓴다.
 *     단, 원내 과정명은 실제 개설 과정과 맞춰 뒀다 — 화면끼리 말이 어긋나면 안 된다.
 *   ✅ 실데이터로 바꾸려면: 수집기(주 1회 배치) → 공고 저장 → 직무별 역량 추출(AI)
 *     → 훈련생 이수 이력과 대조. 이 파일은 손대지 않아도 된다.
 *
 * ■ 서버 연동 지점
 *   window._serverRoadmap = { collectedAt: '2026-07-27', jobs: [...] }
 *   대입 스크립트는 이 파일보다 **앞에** 둔다. 뒤에 두면 서버 데이터가 영원히
 *   더미에 가려진다 (CLAUDE.md 규칙 4).
 */
(function () {
    'use strict';
        ...
        jobs: [
            {
                ...
                steps: [
                    ...
                    },
                    {
                        ...
                        reason: '보완이 필요한 역량 중 요구 빈도가 가장 높아요. 원내 모집중 과정 "클라우드 인프라 입문 (AWS)" 이 이 단계에 해당해요.',
                        status: 'locked', icon: '🐳'
                    },
                    {
                        title: 'AWS 배포 경험 만들기',
            ...
            },
            {
                ...
                steps: [
                    ...
                    },
                    {
                        ...
                        reason: '원내 모집중 과정 "데이터 분석 실무 (Python/Pandas)" 를 먼저 들으면 이어집니다.',
                        status: 'locked', icon: '🤖'
                    }
                ],
                postings: [
